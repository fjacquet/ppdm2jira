...
```powershell
$ActiveDirectoryIdentityProviderResponse = Initialize-PpdmApiReferenceActiveDirectoryIdentityProviderResponse  -AvailableSince null `
 -Config null `
 -Description null `
 -Enabled null `
 -VarHost null `
...
```powershell
$ActiveDirectoryIdentityProviderResponse | ConvertTo-JSON
```
