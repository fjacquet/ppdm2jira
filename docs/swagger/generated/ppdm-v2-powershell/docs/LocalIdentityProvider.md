...
**Enabled** | **Boolean** | Indicates whether the local identity provider service is enabled. | [optional] [default to $true]
...
**Port** | **Int32** | A port override if different from the default one for the referenced identity service | [optional] 
...
**TotalAccounts** | **Int32** | Number of Local Identity Provider accounts. | [optional] 

## Examples

- Prepare the resource
```powershell
$LocalIdentityProvider = Initialize-PpdmApiReferenceLocalIdentityProvider  -Description null `
 -Enabled null `
 -VarHost null `
 -Link null `
 -Locator null `
...
```powershell
$LocalIdentityProvider | ConvertTo-JSON
```
