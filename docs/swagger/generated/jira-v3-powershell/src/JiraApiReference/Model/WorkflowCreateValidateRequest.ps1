#
# The Jira Cloud platform REST API
# Jira Cloud platform REST API documentation
# Version: 1001.0.0-SNAPSHOT-6617cb5f5c8ebc57da84ed8a4b40f48d8d36f5a1
# Contact: ecosystem@atlassian.com
# Generated by OpenAPI Generator: https://openapi-generator.tech
#

<#
.SYNOPSIS

No summary available.

.DESCRIPTION

No description available.

.PARAMETER Payload
No description available.
.PARAMETER ValidationOptions
No description available.
.OUTPUTS

WorkflowCreateValidateRequest<PSCustomObject>
#>

function Initialize-WorkflowCreateValidateRequest {
    [CmdletBinding()]
    Param (
        [Parameter(Position = 0, ValueFromPipelineByPropertyName = $true)]
        [PSCustomObject]
        ${Payload},
        [Parameter(Position = 1, ValueFromPipelineByPropertyName = $true)]
        [PSCustomObject]
        ${ValidationOptions}
    )

    Process {
        'Creating PSCustomObject: JiraApiReference => WorkflowCreateValidateRequest' | Write-Debug
        $PSBoundParameters | Out-DebugParameter | Write-Debug

        if ($null -eq $Payload) {
            throw "invalid value for 'Payload', 'Payload' cannot be null."
        }


        $PSO = [PSCustomObject]@{
            'payload' = ${Payload}
            'validationOptions' = ${ValidationOptions}
        }


        return $PSO
    }
}

<#
.SYNOPSIS

Convert from JSON to WorkflowCreateValidateRequest<PSCustomObject>

.DESCRIPTION

Convert from JSON to WorkflowCreateValidateRequest<PSCustomObject>

.PARAMETER Json

Json object

.OUTPUTS

WorkflowCreateValidateRequest<PSCustomObject>
#>
function ConvertFrom-JsonToWorkflowCreateValidateRequest {
    Param(
        [AllowEmptyString()]
        [string]$Json
    )

    Process {
        'Converting JSON to PSCustomObject: JiraApiReference => WorkflowCreateValidateRequest' | Write-Debug
        $PSBoundParameters | Out-DebugParameter | Write-Debug

        $JsonParameters = ConvertFrom-Json -InputObject $Json

        # check if Json contains properties not defined in WorkflowCreateValidateRequest
        $AllProperties = ('payload', 'validationOptions')
        foreach ($name in $JsonParameters.PsObject.Properties.Name) {
            if (!($AllProperties.Contains($name))) {
                throw "Error! JSON key '$name' not found in the properties: $($AllProperties)"
            }
        }

        If ([string]::IsNullOrEmpty($Json) -or $Json -eq "{}") { # empty json
            throw "Error! Empty JSON cannot be serialized due to the required property 'payload' missing."
        }

        if (!([bool]($JsonParameters.PSobject.Properties.name -match 'payload'))) {
            throw "Error! JSON cannot be serialized due to the required property 'payload' missing."
        } else {
            $Payload = $JsonParameters.PSobject.Properties['payload'].value
        }

        if (!([bool]($JsonParameters.PSobject.Properties.name -match 'validationOptions'))) { #optional property not found
            $ValidationOptions = $null
        } else {
            $ValidationOptions = $JsonParameters.PSobject.Properties['validationOptions'].value
        }

        $PSO = [PSCustomObject]@{
            'payload' = ${Payload}
            'validationOptions' = ${ValidationOptions}
        }

        return $PSO
    }

}

