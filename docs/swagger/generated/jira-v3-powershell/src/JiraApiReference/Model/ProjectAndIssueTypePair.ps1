#
# The Jira Cloud platform REST API
# Jira Cloud platform REST API documentation
# Version: 1001.0.0-SNAPSHOT-6617cb5f5c8ebc57da84ed8a4b40f48d8d36f5a1
# Contact: ecosystem@atlassian.com
# Generated by OpenAPI Generator: https://openapi-generator.tech
#

<#
.SYNOPSIS

No summary available.

.DESCRIPTION

A project and issueType ID pair that identifies a status mapping.

.PARAMETER IssueTypeId
The ID of the issue type.
.PARAMETER ProjectId
The ID of the project.
.OUTPUTS

ProjectAndIssueTypePair<PSCustomObject>
#>

function Initialize-ProjectAndIssueTypePair {
    [CmdletBinding()]
    Param (
        [Parameter(Position = 0, ValueFromPipelineByPropertyName = $true)]
        [String]
        ${IssueTypeId},
        [Parameter(Position = 1, ValueFromPipelineByPropertyName = $true)]
        [String]
        ${ProjectId}
    )

    Process {
        'Creating PSCustomObject: JiraApiReference => ProjectAndIssueTypePair' | Write-Debug
        $PSBoundParameters | Out-DebugParameter | Write-Debug

        if ($null -eq $IssueTypeId) {
            throw "invalid value for 'IssueTypeId', 'IssueTypeId' cannot be null."
        }

        if ($null -eq $ProjectId) {
            throw "invalid value for 'ProjectId', 'ProjectId' cannot be null."
        }


        $PSO = [PSCustomObject]@{
            'issueTypeId' = ${IssueTypeId}
            'projectId' = ${ProjectId}
        }


        return $PSO
    }
}

<#
.SYNOPSIS

Convert from JSON to ProjectAndIssueTypePair<PSCustomObject>

.DESCRIPTION

Convert from JSON to ProjectAndIssueTypePair<PSCustomObject>

.PARAMETER Json

Json object

.OUTPUTS

ProjectAndIssueTypePair<PSCustomObject>
#>
function ConvertFrom-JsonToProjectAndIssueTypePair {
    Param(
        [AllowEmptyString()]
        [string]$Json
    )

    Process {
        'Converting JSON to PSCustomObject: JiraApiReference => ProjectAndIssueTypePair' | Write-Debug
        $PSBoundParameters | Out-DebugParameter | Write-Debug

        $JsonParameters = ConvertFrom-Json -InputObject $Json

        # check if Json contains properties not defined in ProjectAndIssueTypePair
        $AllProperties = ('issueTypeId', 'projectId')
        foreach ($name in $JsonParameters.PsObject.Properties.Name) {
            if (!($AllProperties.Contains($name))) {
                throw "Error! JSON key '$name' not found in the properties: $($AllProperties)"
            }
        }

        If ([string]::IsNullOrEmpty($Json) -or $Json -eq "{}") { # empty json
            throw "Error! Empty JSON cannot be serialized due to the required property 'issueTypeId' missing."
        }

        if (!([bool]($JsonParameters.PSobject.Properties.name -match 'issueTypeId'))) {
            throw "Error! JSON cannot be serialized due to the required property 'issueTypeId' missing."
        } else {
            $IssueTypeId = $JsonParameters.PSobject.Properties['issueTypeId'].value
        }

        if (!([bool]($JsonParameters.PSobject.Properties.name -match 'projectId'))) {
            throw "Error! JSON cannot be serialized due to the required property 'projectId' missing."
        } else {
            $ProjectId = $JsonParameters.PSobject.Properties['projectId'].value
        }

        $PSO = [PSCustomObject]@{
            'issueTypeId' = ${IssueTypeId}
            'projectId' = ${ProjectId}
        }

        return $PSO
    }

}

