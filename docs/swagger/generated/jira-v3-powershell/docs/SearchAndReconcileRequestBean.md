...
**ReconcileIssues** | **Int64[]** | Strong consistency issue ids to be reconciled with search results. Accepts max 50 ids. This list of ids should be consistent with each paginated request across different pages. | [optional] 

## Examples

- Prepare the resource
```powershell
$SearchAndReconcileRequestBean = Initialize-JiraApiReferenceSearchAndReconcileRequestBean  -Expand null `
 -Fields null `
 -FieldsByKeys null `
 -Jql null `
 -MaxResults null `
...
```powershell
$SearchAndReconcileRequestBean | ConvertTo-JSON
```
