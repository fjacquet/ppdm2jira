...
**ReleasedProjectKeys** | **String[]** | Previous project keys to be released from the current project. Released keys must belong to the current project and not contain the current project key | [optional] 
...
```powershell
$UpdateProjectDetails = Initialize-JiraApiReferenceUpdateProjectDetails  -AssigneeType null `
 -AvatarId null `
 -CategoryId null `
 -Description null `
